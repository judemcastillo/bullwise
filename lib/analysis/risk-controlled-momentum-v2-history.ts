...
export const RISK_CONTROLLED_MOMENTUM_V2_HISTORY_VERSION = "1.0.0";
export const RISK_CONTROLLED_MOMENTUM_V2_HISTORY_POLICY = {
	provider: "alpaca",
	feed: "sip",
	adjustment: "all",
	interval: "1d",
	...
	outputPath:
		"artifacts/analysis/analysis-risk-controlled-momentum-v2-history.json",
	overwrite: false,
} as const;
...
function validateMarketData(input: {
	value: MarketBars;
	symbol: string;
}) {
	const { value, symbol } = input;
	if (
		value.provider !== RISK_CONTROLLED_MOMENTUM_V2_HISTORY_POLICY.provider ||
		value.providerSymbol !== symbol ||
		value.currency !== "USD" ||
		value.interval !== RISK_CONTROLLED_MOMENTUM_V2_HISTORY_POLICY.interval ||
		value.adjusted !== true ||
		value.timeliness !== "historical" ||
		value.from.toISOString() !==
			RISK_CONTROLLED_MOMENTUM_V2_HISTORY_POLICY.requestedFrom ||
		value.to.toISOString() !== RISK_CONTROLLED_MOMENTUM_V2_HISTORY_POLICY.requestedTo
	) {
		throw new Error(`${symbol} does not match the frozen history provenance`);
	}
	if (value.bars.length === 0) throw new Error(`${symbol} returned no daily bars`);
	const lower = Date.parse(RISK_CONTROLLED_MOMENTUM_V2_HISTORY_POLICY.requestedFrom);
	const upper = Date.parse(RISK_CONTROLLED_MOMENTUM_V2_HISTORY_POLICY.requestedTo);
...
export function validateRiskControlledMomentumV2FetchArguments(
	...
	if (arguments_.length > 0 && !help) {
		throw new Error("The frozen v2 history fetch accepts no overrides");
	}
	return { help };
}

export function buildRiskControlledMomentumV2HistoryArtifact(input: {
	...
	const expectedSymbols = [
		RISK_CONTROLLED_MOMENTUM_V2_HISTORY_POLICY.benchmarkSymbol,
		...RISK_CONTROLLED_MOMENTUM_V2_SYMBOLS,
	];
	if (
	...
	const benchmarkSymbol = RISK_CONTROLLED_MOMENTUM_V2_HISTORY_POLICY.benchmarkSymbol;
	return {
		schemaVersion: RISK_CONTROLLED_MOMENTUM_V2_HISTORY_VERSION,
		...
		requested: { ...RISK_CONTROLLED_MOMENTUM_V2_HISTORY_POLICY },
...
export function serializeRiskControlledMomentumV2HistoryArtifact(
	artifact: ReturnType<typeof buildRiskControlledMomentumV2HistoryArtifact>,
) {
	return `${JSON.stringify(artifact, null, 2)}\n`;
}

export async function writeRiskControlledMomentumV2HistoryArtifact(input: {
	path: string;
	artifact: ReturnType<typeof buildRiskControlledMomentumV2HistoryArtifact>;
}) {
	const contents = serializeRiskControlledMomentumV2HistoryArtifact(input.artifact);
