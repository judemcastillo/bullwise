...
import { TRANSPARENT_ANALYSIS_AI_CONTENT_EVALUATION_V1_3_PROTOCOL } from "@/lib/analysis/transparent-analysis-ai-content-evaluation-v1-3";
...
const OUTPUT = "artifacts/analysis/transparent-analysis-ai-content-evaluation-v1-3.json";
const USAGE = `Usage: npm run evaluate:transparent-analysis-ai-content-v1-3

Runs the preregistered Gemini 3.5 Flash-Lite v1.3 development evaluation once.
Returned outputs are evaluated separately from provider completion, and the local
fallback remains mandatory. No Bullwise timeout, pacing delay, retry, production
integration, or overwrite is allowed. Requires GEMINI_API_KEY.`;
...
async function main() {
	...
	const report = {
		version: TRANSPARENT_ANALYSIS_AI_CONTENT_EVALUATION_V1_3_PROTOCOL.version,
	...
	console.log(
		`${candidate.model}: ${candidate.decision} | ${candidate.automatedPassed}/10 automated gates | ${candidate.observations.providerCompleted}/${candidate.generationFixtureCount} completed`,
	);
}

main().catch((error: unknown) => {
	if (error && typeof error === "object" && "code" in error && error.code === "EEXIST") {
		console.error("The v1.3 content-evaluation report already exists; it was not replaced.");
	} else {
